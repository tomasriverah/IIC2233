...
def obtener_programones(lineas): 
    ...
    # Completar
    programones = list(map(lambda x: Programon(*(x.split(','))), lineas))
    return programones
...
def obtener_tipos(programones): 
    ...
    repeticiones.'''
    # Completar
    tipos = [[x[2], x[3]] for x in programones]
    lista_aplanada = reduce(lambda x, y: x + y, tipos)
    sett = set(lista_aplanada)
    lista_2 = list(sett)
    lista_2.remove('')
    return lista_2
...
def obtener_programones_de_tipo(programones, tipo):
    ...
    programones_tipo = [programon for programon in programones if programon[2] == tipo or
                        programon[3] == tipo]
    return programones_tipo
...
def obtener_caracteristica_promedio(programones, caracteristica): 
    ...
    # Completar
    numeros = [int(getattr(x,caracteristica)) for x in programones]
    suma = sum(numeros)
